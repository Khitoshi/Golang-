...
type Todo struct {
	...
	Until     time.Time `bun:"numtil,nullzero"form:"until"`
	CreatedAt time.Time
...
func main() {
	...
	//dbを開く
	//sqldb, err := sql.Open("postgres", os.Getenv("postgres://postgres:985632@localhost:5432/webapp_todos?"))
	sqldb, err := sql.Open("postgres", "user=postgres dbname=webapp_todos password=985632 sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	defer sqldb.Close()

	//sqldbをラップしてデータベースハンドラー取得
	//sqldbはデータベース
	//pgdialect.New()はデータベースの種類
